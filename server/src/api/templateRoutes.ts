...
export default class TemplateRoutes {
    ...
    constructor(
        ...
        private _defaultHeader: Record<string, string> = {
    ...
    protected _route<ReqBody = unknown, ResBody = unknown, P = RouteParameters<string>>(
        method: RouteMethod,
        route: string,
        permissions: Array<Permissions> | undefined,
        ...handlers: Array<RequestHandler<P, ResBody, ReqBody>>
    ) {
        return this._app[method](
            route,
            handlers.map(
                (handler) => (req: Request<P, ResBody, ReqBody>, res: Response<ResBody>, next: NextFunction) =>
                    this._wrapper<P, ResBody, ReqBody>(permissions, req, res, next, handler)
            )
        );
    }

    private async _wrapper<P, ResBody, ReqBody>(
        permissions: Array<Permissions> | undefined,
        request: Request<P, ResBody, ReqBody>,
    ...
    private _errorHandler<P, ResBody, ReqBody, ReqQuery>(
        error: unknown,
